...
class VGG:
    ...
    def __init__(self, input_dim=(1, 28, 28), output_size=10, dropout_ratio=0, use_batchnorm=False, back_eta=True):
        # 가중치 초기화===========
        param_size = [
            {'filter_num': 64, 'filter_size': 3, 'pad': 1, 'stride': 1},  # 64, 28, 28
            {'filter_num': 64, 'filter_size': 3, 'pad': 1, 'stride': 1},  # 64, 28, 28
            {'pooling': 2, 'stride': 2},  # 64, 14, 14
            {'filter_num': 128, 'filter_size': 3, 'pad': 1, 'stride': 1},  # 128, 14, 14
            {'filter_num': 128, 'filter_size': 3, 'pad': 2, 'stride': 1},  # 128, 16, 16
            {'pooling': 2, 'stride': 2},  # 128, 8, 8
            {'filter_num': 256, 'filter_size': 3, 'pad': 1, 'stride': 1},  # 256, 8, 8
            {'filter_num': 256, 'filter_size': 3, 'pad': 1, 'stride': 1},
            {'filter_num': 256, 'filter_size': 3, 'pad': 1, 'stride': 1},
            {'pooling': 2, 'stride': 2},  # 128, 4, 4
            {'filter_num': 512, 'filter_size': 3, 'pad': 1, 'stride': 1},  # 512, 4, 4
            {'filter_num': 512, 'filter_size': 3, 'pad': 1, 'stride': 1},
            {'filter_num': 512, 'filter_size': 3, 'pad': 1, 'stride': 1},
            {'pooling': 2, 'stride': 2},  # 256, 2, 2
            {'filter_num': 512, 'filter_size': 3, 'pad': 1, 'stride': 1},  # 512, 2, 2
            {'filter_num': 512, 'filter_size': 3, 'pad': 1, 'stride': 1},
            {'filter_num': 512, 'filter_size': 3, 'pad': 1, 'stride': 1},
        ...
        for _, ps in enumerate(param_size):
            if 'filter_num' in ps:  # Convolutional Layer 초기화
                ...
                self.params['W' + str(i)] = scale * np.random.randn(filter_num, pre_channel_num, filter_size, filter_size)
